...
export default function InputQuantity({
  ...
  const handleBlurOrMouseLeave = (event) => {
   ...
   // **Call the roundOffCalculator here if RoundOff and RoundType are provided**
   if (newValue !== null && RoundOff != null && !isNaN(RoundOff) && RoundType) {
    const originalValue = newValue;
    const roundedValue = roundOffCalculator(newValue, RoundOff, RoundType, decimalPart);

    // Check if rounding changed the value
    if (roundedValue !== newValue) {
      // Check if minimum and maximum constraints are satisfied after rounding
      if (
  ...
  // Call roundOffCalculator if RoundOff and RoundType are provided
  if (newValue !== null && RoundOff != null && !isNaN(RoundOff) && RoundType) {
    const originalValue = newValue;
    const roundedValue = roundOffCalculator(newValue, RoundOff, RoundType, decimalPart);

    if (roundedValue !== newValue) {
      // Check if the rounded value respects min/max constraints
      if (
        (MinimumValue != null && roundedValue < MinimumValue) ||
        (MaximumValue != null && roundedValue > MaximumValue)
      ) {
