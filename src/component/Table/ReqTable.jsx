...
export default function ReqTable(props) {
  const { rows, IdName, mainDetails, onRowDoubleClick,tableFields } = props;
  ...
  const excludedFields = [
   ...
   // Use tableFields to determine columns
   React.useEffect(() => {
    if (!tableFields || tableFields.length === 0) return;

    // Create columns based on tableFields
    const generatedColumns = tableFields
      .filter((field) => !excludedFields.includes(field.FieldName) && !excludedFields.includes(field.FieldName + "_Name"))
      .map((field) => {
        let dataKey;
        if (field.FieldDisplayType.toLowerCase() === "drop down") {
          // For drop down fields, use FieldName_Name as the data key
          dataKey = field.FieldName + "_Name";
        } else {
          // For other fields, use FieldName as is
        ...
        return {
          ...
          minWidth: 100,
          maxWidth: 200,
          fieldType: field.FieldDisplayType,
        };
      });
      ...
      >
        ...
        >
          ...
          >
            ...
            <Box sx={{pt:"9px",ml:2}}>
            <ActionButton
          iconsClick={props.handleMultiSelection}
                      ...
                      {(popupState) => (
                        <>
                          <TableRow
                            ...
                            onKeyDown={(event) => {
                              if (event.key === "Enter") {
                                // Manually set the anchor and open the popover
