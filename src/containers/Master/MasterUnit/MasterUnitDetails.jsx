      ...
      <Stack spacing={2} sx={{ flex: 1 }}>
        ...
        >
          <Typography underline="hover" sx={style} key="1">
            Unit Details {group === 1 ? "(Add Group)" : group === 2 ? "(Group)" : null }
          </Typography>
        </Breadcrumbs>
      </Stack>
    </div>
...
export default function MasterUnitDetails({
  setPageRender,
  ...
  const handleIconsClick = async (value) => {
    switch (value.trim()) {
      ...
      case "save":
        ...
        if (group === 2 && !mainDetails?.Parent) emptyFields.push("Parent");
  ...
  const handleSave = async () => {
    ...
    const response = await upsertunitmaster(saveData);
    if (response.status === "Success") {
      if (group === 2) {
  ...
  const deleteClick = async () => {
    let response;
    response = await deletetag([{ id: detailPageId }], 14);
    if (response?.status === "Success") {
      showAlert("success", response?.message);
      handleNew();
      const actionExists = userAction.some((action) => action.Action === "New");
