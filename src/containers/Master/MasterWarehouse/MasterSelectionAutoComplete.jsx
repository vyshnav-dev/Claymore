...
export default function MasterSelectionAutoComplete({
  apiKey,
  label,
  autoId,
  formDataName,
  ...
  const handleDataChange = async (newValue) => {
    const update = [...companyList];
    update[index][formDataiId] = newValue ? newValue.Id : null;
    update[index][formDataName] = newValue ? newValue.Name : null;
  
    if (newValue) {
      setCompanyList([...update, { BE: 0, BE_Name: null }]);
    } else {
      if (companyList?.length <= 1) {
        setCompanyList([{ BE: 0, BE_Name: null }]);
      }else if(companyList?.length-1 === index){
        const update = [...companyList];
        update[index][formDataiId] =  null;
        update[index][formDataName] =null;
  ...
  const debouncedFetchOptions = debounce(async (searchKey) => {
    ...
    try {
      ...
      // Filter out companies that already exist in `companyList`
      const filteredCompanies = results.filter(company =>
        !companyList.some(filterItem => 
          filterItem.BE === company.Id || filterItem.BE_Name === company.Name
        )
      );
  
      setMenu(filteredCompanies || []);
    } catch {
      setMenu([]);
    }
  }, 300);
    ...
    <>
      <Autocomplete
        ...
        getOptionLabel={(option) =>
          option?.Name  || companyList[index][formDataName] ||""
        }
        value={companyList[index][formDataName] ?? ""}
