...
export default function JobReport({ userAction, disabledDetailed }) {
  ...
  const [rows1, setRows1] = React.useState([]); //To Pass in Table
  const [header, setHeader] = React.useState({}); //To Pass in Table
  ...
  const { getAssignjoborderlist } = inspectionApis();
  const { getjoborderdetailsreport } = reportApis();
  useEffect(() => {
  ...
  const tagDetails = async () => {
    ...
    try {
      if (
        ...
        const response = await getjoborderdetailsreport({
        ...
        if (
          ...
          setRows1(myObject?.AllocatedTo)
          setRows(myObject?.Details);
  ...
  const handleExcel = async () => {
    ...
    const filteredRows = JSON.parse(response?.result)?.Details;
    const filteredHeader =JSON.parse(response?.result)?.Header[0];
    const excelCoverData =JSON.parse(response?.result)?.AllocatedTo;
    await ExcelExport({
            ...
            >
              {Object.keys(header).length > 0 &&
                ...
                <>
                  ...
                  />
                  <UserInputField
                    label={"Ack Submitted Date"}
                    name={"AckSubmittedDate"}
                    type={"date"}
                    disabled={true}
                    mandatory={true}
                    value={header}
              ...
              {rows1?.length > 0 &&
                <JobReportTable rows={rows1} excludedColumns={[]} />
              }
