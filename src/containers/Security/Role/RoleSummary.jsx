...
export default function RoleSummary({ setPageRender, setId, userAction }) {
  ...
  const deleteClick = async () => {
    if (selectedDatas.length === 0) {
      showAlert("info", "Select Role to Delete");
      return;
    }
  ...
  const handledeleteRole = async () => {
    ...
    try {
      let response = await deleterole(deletePayload);
